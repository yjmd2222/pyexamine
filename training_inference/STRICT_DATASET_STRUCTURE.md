...
The training/inference dataset config helper enforces this structure:

`datasets/<individual_dataset>/{excerpt.jsonl, sidecar.jsonl}`
...
- Every child dataset directory must contain **both** files:
  - `excerpt.jsonl`
  - `sidecar.jsonl`
- No fallback names are used in strict mode.
- If any dataset directory is missing either file, config generation fails.
